...
//  CatNap
//
//  Created by Matthew J. Perkins on 10/26/17.
...
class HookBaseNode: SKSpriteNode, EventListenerNode {
    ...
    func didMoveToScene() {
        ...
        let ropeJoint = SKPhysicsJointSpring.joint(
            withBodyA: physicsBody!, bodyB: hookNode.physicsBody!, anchorA: position, anchorB: hookPosition)
    ...
    func hookCat(catPhysicsBody: SKPhysicsBody) {
        ...
        hookJoint = SKPhysicsJointFixed.joint(
            withBodyA: hookNode.physicsBody!, bodyB: catPhysicsBody, anchor: pinPoint)
